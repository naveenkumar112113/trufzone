...
export const seedDatabase = async () => {
  try {
    ...
    let owner = await User.findOne({ phone: '9443182940' });
    if (!owner) {
    ...
    let staffUser = await User.findOne({ phone: '9842100001' } as any);
    if (!staffUser) {
    ...
    let adminUser = await User.findOne({ phone: '9443100002' } as any);
    if (!adminUser) {
    ...
    for (const p of SEED_PLAYERS) {
      let player = await User.findOne({ phone: p.phone });
      if (!player) {
    ...
    // 3. Seed Turfs
    const existingTurfs = await Turf.find({ ownerId: owner._id } as any);
    ...
    for (let dayOffset = -2; dayOffset <= 5; dayOffset++) {
      ...
      for (const turf of turfs) {
        const slotsCount = await TurfSlot.countDocuments({
          turfId: turf._id,
        ...
        if (slotsCount === 0) {
          ...
          await TurfSlot.insertMany(slotsToInsert);
        }
      }
    }
    ...
    if (totalBookingsCount < 15) {
      ...
      const allSlots = await TurfSlot.find({ status: 'AVAILABLE' } as any)
        .sort({ startTime: 1 })
        .limit(40);
      ...
      for (let i = 0; i < bookingTemplates.length && slotIdx < allSlots.length; i++) {
        ...
        if (tmpl.status !== 'CANCELLED') {
          await TurfSlot.updateOne({ _id: slot._id }, { status: 'BOOKED' });
        }
...
export const generateRandomBookings = async (count: number = 5) => {
  try {
    const owner = await User.findOne({ phone: '9443182940' });
    if (!owner) {
      await seedDatabase();
    }
    ...
    for (let i = 0; i < count; i++) {
      ...
      let player = await User.findOne({ name: randName });
      if (!player) {
        player = await User.create({
          name: randName,
          phone: randPhone,
      ...
      // Create slot
      const slot: any = await TurfSlot.create({
        turfId: turf._id,
        startTime: slotDate,
        endTime: slotEndDate,
        price,
